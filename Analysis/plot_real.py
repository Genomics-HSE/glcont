...
sec = 'in2'
#%%
def get_glcont(consensus, model = 0):
    y = np.load(f'glcont/in1_{sec}_model_{model}_consensus_{consensus}_average.npy')
    return y

endo_proportions = [0.5, 0.6, 0.7, 0.8, 0.9, 0.95]#%%
def get_contamix(consensus = 0):
    ...
    for i, cov in enumerate([5, 10, 30]):
        for v in range(1,15):
            for k, prop in enumerate(endo_proportions):
                # print(i, v, k)
                with open(f'contamix/in1_{sec}{cons}/in1_{sec}_cov{cov}_{int(100*prop)}_v{v}_mt.summary.txt') as f:
                    y_cont[i, v, k] = float(
...
def plot_compare(depth):
    ...
    plt.suptitle(f"Haplogroup:{sec}, depth:{depth}", fontsize=14)
    plt.savefig(f"modification_depths{depth}_{sec}.pdf")
    # plt.show()
for d in [5, 10, 30]:
    plot_compare(d)
